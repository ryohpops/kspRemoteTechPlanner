...
module Calculator {
    ...
    export class OrbitalService {
        ...
        period(sma: number, stdGravParam: number) {
            return 2 * Math.PI * Math.sqrt(Math.pow(sma, 3) / stdGravParam);
        }

        nightTime(radius: number, sma: number, stdGravParam: number) {
            return this.period(sma, stdGravParam) * Math.asin((radius / 2) / sma) / Math.PI;
        }

        hohmannStartDV(radius: number, sma1: number, sma2: number, stdGravParam: number) {
            return Math.sqrt(stdGravParam / sma1) * (Math.sqrt((2 * sma2) / (sma1 + sma2)) - 1);
        }

        hohmannFinishDV(radius: number, sma1: number, sma2: number, stdGravParam: number) {
