...
module App.Calculator {
    ...
    export class OrbitalService {
        orbitalPeriod(bodyRadius: number, altitude: number, stdGravParam: number) {
            return 2 * Math.PI * Math.sqrt(Math.pow(bodyRadius + altitude, 3) / stdGravParam);
        }

        orbitalNightTime(bodyRadius: number, altitude: number, stdGravParam: number) {
            var ra: number = bodyRadius + altitude;
            return 2 * Math.pow(ra, 2) / Math.sqrt(ra * stdGravParam) * Math.asin(bodyRadius / ra);
        }

        hohmannStartDeltaV(bodyRadius: number, altitude1: number, altitude2: number, stdGravParam: number) {
        ...
        hohmannFinishDeltaV(bodyRadius: number, altitude1: number, altitude2: number, stdGravParam: number) {
            var r1: number = bodyRadius + altitude1;
            var r2: number = bodyRadius + altitude2;
